...
2. **Palette**: pick 5–7 hex codes. Name each one the way the concept
   ...
   fabric. Do not name them "primary blue" or "neutral 800". Sample
   from references when possible. Required: a foreground and a
   ...
   The palette should support the world the concept committed to —
   if the world is a doctor's clipboard, the palette comes from real
   medical paper, ink, and stamps; if the world is a press proof, it
   comes from registration marks, paper white, and one process color.
...
9. **Tokens file**: write `projects/{slug}/src/styles/brand-tokens.css`
   ...
   global stylesheet. Name the variables descriptively
   (`--ink-iron-gall`, `--paper-newsprint`) — not by role
...
- Color variable names use the *source* of the color, not its role.
  Web Build can map `--color-bg: var(--paper-newsprint)` itself if
